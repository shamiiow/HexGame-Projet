    ...
    def __init__(self, color=None, **kwargs):
        ...
        with self.canvas.before:
            ...
            self.rect = RoundedRectangle(
                size=self.size,
                pos=self.pos,
                radius=[(self.size[0] / 4, self.size[1] / 4)] * 2,
            )
...
class LocalMenuScreen(Screen):
    def __init__(self, **kwargs):
        ...
        self.player1_bot_box = BoxLayout(
            orientation="horizontal", size_hint=(None, None), size=(130, 60)
        )
        ...
        self.footer = GridLayout(
            rows=1, cols=1, padding=[200, 0, 200, 0], size_hint=(1, 0.8)
        )

        self.title = Label(
            text="Local Mode", font_size=90, color=(0 / 255, 20 / 255, 40 / 255, 1)
        )
        ...
        self.texte = Label(
            text="Pick the grid size :",
            font_size=40,
            color=(0 / 255, 20 / 255, 40 / 255, 1),
        )

        self.decrement_button = RoundedButton(
            text="-", font_size=60, size_hint=(0.3, 1)
        )
        self.decrement_button.bind(on_press=self.decrement_grille)
        self.grille_label = ColoredLabel(
            text=str(self.grille),
            color=(0 / 255, 45 / 255, 90 / 255, 1),
            font_size=50,
            size_hint=(0.4, 1),
        )
        self.increment_button = RoundedButton(
            text="+", font_size=60, size_hint=(0.3, 1)
        )
        ...
        self.player1_type_check = Button(
            text="X",
            size_hint=(None, 1),
            width=60,
            background_normal="",
            background_color=(213 / 255, 86 / 255, 90 / 255),
            font_size=40,
        )
        ...
        self.player2_type_check = Button(
            text="X",
            size_hint=(None, 1),
            width=60,
            background_normal="",
        ...
        self.close_button = Button(
            ...
            size_hint=(0.15, 0.13),
            background_normal="",
            background_color=(0 / 255, 45 / 255, 90 / 255, 1),
        )
    ...
    def update_rect(self, *args):
        ...
        self.title.font_size = (Window.size[0] / 800) * 90
        self.texte.font_size = (Window.size[0] / 800) * 40
        self.grille_label.font_size = (Window.size[0] / 800) * 50
        self.decrement_button.font_size = (Window.size[0] / 800) * 60
        self.increment_button.font_size = (Window.size[0] / 800) * 60
        self.play_button.font_size = (Window.size[0] / 800) * 40
        self.close_button.font_size = (Window.size[0] / 800) * 25
    ...
    def go_to_game(self, instance):
        self.manager.get_screen("game_local").set_variables(
        ...
        self.manager.get_screen("game_local").update()

        self.manager.current = "game_local"
