...
class LocalMenuScreen(Screen):
    def __init__(self, **kwargs):
        ...
        self.players_name = GridLayout(padding = 10, size_hint_y=None, height = 0.09*Window.size[1])
        self.players_name.rows = 1
        self.players_name.cols = 2

        self.players_bot = GridLayout(padding = 10, size_hint_y=None, height = 0.09*Window.size[1])
        self.players_bot.rows = 1
        self.players_bot.cols = 4

        self.player1_bot_box = BoxLayout(orientation = 'horizontal', size_hint_x=None, width = 0.1*Window.size[0])
        self.player2_bot_box = BoxLayout(orientation = 'horizontal', size_hint_x=None, width = 0.1*Window.size[0])
        ...
        self.exit_button = Button(text="X", background_color= (0/255, 45/255, 90/255), pos_hint={'x': 0, 'y': 1 - (50/self.float_layout.size[0])}, size_hint = (None,None), size = (50,50))
        ...
        self.player1_entry = TextInput(text='Joueur 1', hint_text="Name player 1", multiline = False, size_hint_x = None, width = 150, background_color = (220/255,134/255,134/255))
        
        self.player2_entry = TextInput(text='Joueur 2', hint_text="Name player 2", multiline = False, size_hint_x = None, width = 150, background_color = (126/255,215/255,193/255))
        
        self.player1_type_check = CheckBox(size_hint=(None, None), size=(40, 40), color = (0, 0, 1, 1))
        self.player1_type_label = Label(text="bot ?", color = (0,0,0))
        
        self.player2_type_label = Label(text="bot ?", color = (0,0,0))
        self.player2_type_check = CheckBox(size_hint=(None, None), size=(40, 40), color = (0, 0, 1, 1))
        
        self.grid_size_minus = Button(text = "-", background_color= (56/255, 62/255, 153/255), background_normal='')
        self.grid_size = ColoredLabel(text='Hex Game', color = [56/255, 62/255, 153/255])
        self.grid_size_plus = Button(text = "+", background_color= (56/255, 62/255, 153/255), background_normal='')
    ...
    def update_rect(self, instance, value):
        value = 20
        self.players_name.spacing = Window.size[0]- self.player2_entry.width - self.player1_entry.width - 2*self.players_name.padding[0]
        self.players_bot.spacing = Window.size[0]- self.player1_bot_box.width - self.player2_bot_box.width - 2*self.players_bot.padding[0]
        self.footer.padding = [Window.size[0]*0.3,0]*2
        self.hex_grid_size.padding = [Window.size[0]*0.32,Window.size[1]*0.12,Window.size[0]*0.32,Window.size[1]*0.04]
