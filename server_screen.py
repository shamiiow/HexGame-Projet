...
class ServerScreen(Screen):
    def __init__(self, **kwargs):
        ...
        self.content_layout = BoxLayout(
            orientation="vertical", padding=[30, 30, 30, 30], spacing=10
        )
        ...
        self.input_layout = BoxLayout(
            orientation="horizontal", size_hint=(1, 0.1), spacing=100
        )
    ...
    def update_rects(self, instance, value):
        if self.varParceQuePythonEstNul == 0:
            self.varParceQuePythonEstNul = 1

        elif self.varParceQuePythonEstNul == 1:
            self.varParceQuePythonEstNul = 2

        elif self.varParceQuePythonEstNul == 2:
            self.varParceQuePythonEstNul = 3
    ...
    def go_to_waiting_join(self, instance):
        self.manager.get_screen("waiting").set_variables(
    ...
    def print_server(self):
        ...
        for host in self.game_host:
            ...
            line_layout.bind(
                size=self.update_line_rect(rect), pos=self.update_line_rect(rect)
            )

            label = Label(text=host, size_hint=(None, 1), width=200)
            join_button = RoundedButton(
                text="Join", size_hint=(None, None), width=100, height=40
            )  # Taille réduite du bouton "Join"
            join_button.bind(
                on_press=lambda instance, numServ=host: self.go_to_x(instance, numServ)
            )
            button_container = BoxLayout(
                orientation="vertical", size_hint=(None, 1), width=100
            )
